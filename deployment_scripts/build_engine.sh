...
/usr/src/tensorrt/bin/trtexec \
    ...
    --minShapes=images:${MIN_BATCH}x${IMAGE_CHANNELS}x${IMAGE_SIZE1}x${IMAGE_SIZE2},img_masks:${MIN_BATCH}x${NUM_IMAGES},lang_tokens:${MIN_BATCH}x${MAX_SEQ_LEN},lang_masks:${MIN_BATCH}x${MAX_SEQ_LEN},state:${MIN_BATCH}x${STATE_DIM},noise:${MIN_BATCH}x${ACTION_HORIZON}x${ACTION_DIM} \
    --optShapes=images:${OPT_BATCH}x${IMAGE_CHANNELS}x${IMAGE_SIZE1}x${IMAGE_SIZE2},img_masks:${OPT_BATCH}x${NUM_IMAGES},lang_tokens:${OPT_BATCH}x${MAX_SEQ_LEN},lang_masks:${OPT_BATCH}x${MAX_SEQ_LEN},state:${OPT_BATCH}x${STATE_DIM},noise:${OPT_BATCH}x${ACTION_HORIZON}x${ACTION_DIM} \
    --maxShapes=images:${MAX_BATCH}x${IMAGE_CHANNELS}x${IMAGE_SIZE1}x${IMAGE_SIZE2},img_masks:${MAX_BATCH}x${NUM_IMAGES},lang_tokens:${MAX_BATCH}x${MAX_SEQ_LEN},lang_masks:${MAX_BATCH}x${MAX_SEQ_LEN},state:${MAX_BATCH}x${STATE_DIM},noise:${MAX_BATCH}x${ACTION_HORIZON}x${ACTION_DIM} \
    > "${ENGINE_PATH}.log" 2>&1
...
echo "  images: [${MIN_BATCH}x${IMAGE_CHANNELS}x${IMAGE_SIZE1}x${IMAGE_SIZE2}] to [${MAX_BATCH}x${IMAGE_CHANNELS}x${IMAGE_SIZE}x${IMAGE_SIZE}]"
